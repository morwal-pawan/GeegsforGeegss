...
public class StringPro22A {
    ...
    static int maxSumSquar(String str, int k) {
	...
	while (k > 0) {
	    Node temp = pq.poll();
	    temp.freq--;
	    if (temp.freq >= 0) // to handle the negative
		pq.add(temp);
	    k--;
	}
	int squaSum = 0;
	...
	System.out.println(" eventNumberSubString : " + maxSumSquar(str1, K));
	br.close();
